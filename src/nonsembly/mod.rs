...
// 	pub fn new(script: &'a InstrBlock) -> Self {
// 		Self {
// 			script,
// 			errors: Vec::new(),
...
// 		match instr {
// 			Instruction::FunctionDef { name, type_def: value, args, body, span } => {
...
// 				self.push_var(VarData::new(name, ret_type.unwrap_or(TypeDef::NonTyped(NonTypeReason::ValidateError)), span));
// 			},
...
// 					return;
// 				};
// 				if matches!(var.value, TypeDef::NonTyped(_)) {
// 					return;
// 				}
...
// 							(TypeDef::Array(i), t) | (t, TypeDef::Array(i)) if *t == **i => (),
// 							(TypeDef::String, TypeDef::Num) | (TypeDef::Num, TypeDef::String) => (),
// 							(a, b) if a == b => (),
// 							(a, b) => self.errors.push(Error::BadAddType { a: a.clone(), b: b.clone(), span }),
// 						}
// 					},
// 					_ => {
// 						if var.value != TypeDef::Num {
// 							self.errors.push(Error::BadType { expected: Some(TypeDef::Num), got: var.value.clone(), span });
// 						}
// 						if assign_type != TypeDef::Num {
// 							self.errors.push(Error::BadType { expected: Some(TypeDef::Num), got: assign_type, span });
// 						}
// 					}
// 				}
// 			},
...
// 	fn validate_function(&mut self, func_name: &'a String, body: &'a [Instruction], func_value: &'a TypeDef, args: &'a [(String, TypeDef, Span)], span: &'a Span) {
// 		self.push_ctx_func();
...
// 				let return_type = match return_type.as_ref().map(|value| self.resolve_type(&value)).unwrap_or(Ok(TypeDef::Null)) {
// 					Ok(value) => value,
// 					Err(_) => continue,
// 				};
...
// 					(Ok(a), Some(Ok(b))) => (a, b),
// 					(Ok(a), None) => (a, TypeDef::Null),
// 					_ => return,
// 				};

// 				match (op.deref(), (a, b)) {
// 					(Operation::And(..), (bt, TypeDef::Bool)) |
// 					(Operation::Or(..), (bt, TypeDef::Bool)) |
// 					(Operation::And(..), (TypeDef::Bool, bt)) |
// 					(Operation::Or(..), (TypeDef::Bool, bt))
// 						if bt != TypeDef::Bool => self.errors.push(Error::BadType { expected: Some(TypeDef::Bool), got: bt, span }),
...
// 					(Operation::Div(..), (a, b)) |
// 					(Operation::Mod(..), (a, b))
// 						if a != TypeDef::Num || b != TypeDef::Num =>
// 						self.errors.push(Error::BadType { expected: Some(TypeDef::Num), got: if a != TypeDef::Num { a } else { b }, span }),

// 					(Operation::Incr(..), (bt, _)) |
// 					(Operation::Decr(..), (bt, _))
// 						if bt != TypeDef::Num => self.errors.push(Error::BadType { expected: Some(TypeDef::Num), got: bt, span }),
...
// 						},
// 					(Operation::Index(..), (TypeDef::Array(_), i))
// 						if i != TypeDef::Num => self.errors.push(Error::BadType { expected: Some(TypeDef::Num), got: i, span }),
// 					(Operation::Index(..), (TypeDef::Map(kv), i))
// 						if (*kv).0 != i => self.errors.push(Error::BadType { expected: Some((*kv).0.clone()), got: i, span }),
// 					(Operation::Index(..), (t, _))
// 						if !matches!(t, TypeDef::Array(_) | TypeDef::Map(_)) => self.errors.push(Error::CantBeIndexed { ty: t, span }),
// 					_ => (),
// 				}
// 			},
// 		}
...
// 				self.validate_value(cond, span);
// 				if let Ok(cond_type) = self.resolve_type(&cond) && cond_type != TypeDef::Bool {
// 					self.errors.push(Error::BadType { expected: Some(TypeDef::Bool), got: cond_type, span });
// 				}
...
// 					if let TypeDef::Array(inner) = iter_type {
...
// 				(true, true) => Ok(TypeDef::Array(inner.as_ref().unwrap().clone().into())),
// 				(false, false) => Result::map(self.resolve_type(items.first().unwrap()), |t| TypeDef::Array(Box::new(t))),
...
// 				(_, true) => Ok(TypeDef::Map(inner.as_ref().unwrap().clone().into())),
// 				(false, false) => {
// 					let (k, v) = items.first().unwrap();
// 					let (k, v) = (self.resolve_type(k)?, self.resolve_type(v)?);
// 					Ok(TypeDef::Map((k, v).into()))
// 				},
// 				(true, false) => Err(NonTypeReason::ValidateError),
// 			},
...
// 				match (a, b) {
// 					(a, b) if a == b => Ok(a),
// 					(TypeDef::Array(t), _) | (_, TypeDef::Array(t)) => Ok(TypeDef::Array(Box::new(*t))),
// 					(TypeDef::String, TypeDef::Num) | (TypeDef::Num, TypeDef::String) => Ok(TypeDef::String),
...
// 				match t {
// 					TypeDef::Array(t) => Ok(*t),
// 					TypeDef::Map(kv) => Ok((*kv).0),
...
// impl<'a> FuncData<'a> {
// 	pub fn new(name: &'a str, args: Vec<&'a TypeDef>, value: &'a TypeDef, span: &'a Span) -> Self {
...
// impl<'a> PartialEq<TypeDef> for FuncData<'a> {
...
// impl<'a> VarData<'a> {
// 	pub fn new(name: &'a str, value: TypeDef, span: &'a Span) -> Self {
...
// impl<'a> PartialEq<TypeDef> for VarData<'a> {
// 	fn eq(&self, other: &TypeDef) -> bool {
// 		self.value == *other
// 	}
// }
